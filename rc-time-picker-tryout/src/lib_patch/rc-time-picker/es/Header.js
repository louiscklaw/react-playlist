// patched for accept FW HW characters
...
function FwToHwNumeric(fw_number_in) {
  let char_code = fw_number_in.charCodeAt();
  return String.fromCharCode(char_code - ASCII_FW_0 + ASCII_0);
}

function sanitizeFullWidthNumber(test_input) {
  if (typeof test_input === 'string') {
    return test_input
      .split('')
      .map((original_char) => {
    ...
    console.log('sanitizeFullWidthNumber', 'not a string, bypass');
    return test_input;
  }
}
  ...
  (function (_Component) {
    ...
    function Header(props) {
      ...
      _this = _possibleConstructorReturn(
        this,
        _getPrototypeOf(Header).call(this, props)
      );
      ...
      _defineProperty(
        ...
        'onInputChange',
        function (event) {
          var str = sanitizeFullWidthNumber(event.target.value);
          console.log(
            'onInputChange',
            sanitizeFullWidthNumber(event.target.value)
          );
      ...
      },
      {
        ...
        value: function getInput() {
          ...
          var invalidClass = invalid
            ? ''.concat(prefixCls, '-input-invalid')
            : '';
