...
function CountdownByMinutes(props) {
  ...
  let [datetime_alarm, setDatetimeAlarm] = React.useState(
    default_value.datetime_alarm
  );
  let [alarm_due, setAlarmDue] = React.useState(default_value.alarm_due);
  let [alarm_enable, setAlarmEnable] = React.useState(
    default_value.alarm_enable
  );
  let [time_remaining, setTimeRemaining] = React.useState(
    default_value.time_remaining
  );
  let [times_up, setTimesUp] = React.useState(default_value.times_up);

  let [alert_enable, setAlertEnable] = React.useState(
    default_value.alert_enable
  );
  let [alert_remaining, setAlertRemaining] = React.useState(
    default_value.alert_remainig_seconds
  );
  ...
  const handleSetOnClick = (e) => {
    ...
    let value_minutes = document.querySelector(
      '#countdown-input-minutes'
    ).value;
    let value_seconds = document.querySelector(
      '#countdown-input-seconds'
    ).value;
  ...
  React.useEffect(() => {
    if (alarm_enable) {
      let remaining_seconds = Interval.fromDateTimes(
        datetime_now,
        datetime_alarm
      );
  ...
  return (
    <>
      ...
      <input
        id="countdown-input-minutes"
        onChange={handleCountdownOnChange}
        defaultValue={countdown_minutes}
      />
      <input
        id="countdown-input-seconds"
        onChange={handleCountdownOnChange}
      ...
      <table>
        <tbody>
          <tr>
            <td>Timenow</td>
            <td>{JSON.stringify(datetime_now)}</td>
          </tr>
          <tr>
            <td>Alarm will occur in: </td>
            <td>{JSON.stringify(datetime_alarm)}</td>
          </tr>
          <tr>
            <td>Time remaining: </td>
            <td>{JSON.stringify(time_remaining)} seconds</td>
          </tr>
          <tr>
            <td>Alarm Due: </td>
            <td>{JSON.stringify(alarm_due)}</td>
          </tr>
          <tr>
            <td>Alarm Enable: </td>
            <td>{JSON.stringify(alarm_enable)}</td>
          </tr>
          <tr>
            <td>Alert Enable: </td>
            <td>{JSON.stringify(alert_enable)}</td>
          </tr>
        </tbody>
      </table>
    </>
